...
class ArticleGenerator:
    ...
    def generate_review_article(self, item: Dict[str, Any]) -> str:
        ...
        prompt = f"""以下のAmazonの商品情報を元に、ブログ「はてなブログ」に投稿するための、高品質で知的、かつ具体的なレビュー・紹介記事を執筆してください。
ただ長いだけの冗長な文章や、同じ接続詞・言い回しの繰り返しを徹底的に排除し、読者が本当に知りたいスペック、実際のユースケース、購入メリットを洗練された日本語で書いてください。
...
（商品のターゲット層や、現代社会のどのような課題を解決できる製品かを解説）

## 2. デザインと携帯性：洗練されたスタイルと使いやすさ
（サイズ感、重量、質感、持ち歩きやすさなどの外観とユーザビリティについて詳しく説明）

## 3. 実力検証：使ってわかった圧倒的なパフォーマンス
（主な特徴に記載されている機能が、実際の日常生活やワークスタイルでどのようにメリットをもたらすか、具体的なユースケースを交えて解説）

## 4. 本音でレビュー：メリットと購入前に知っておきたい注意点
（素晴らしい点だけでなく、価格や仕様、あるいは競合製品と比較した際の注意点やデメリットも客観的に説明）

## 5. まとめ：どんな人におすすめ？
（この商品を購入して最も生活やワークスタイルがアップグレードされる人（ターゲット層）を提案）
...
5. AI特有の「〜はいかがでしょうか」「〜をご紹介します」「〜と言えるでしょう」といったテンプレフレーズを多用せず、人間がレビューしたかのようなリアリティとキレのある文体で書いてください。
"""
