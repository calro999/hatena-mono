...
class ArticleGenerator:
    ...
    def generate_review_article(self, item: Dict[str, Any]) -> str:
        ...
        prompt = f"""以下の商品の情報を元に、はてなブログの「編集部おすすめ」に選出されるレベルの、熱量が高く独自の視点に満ちた個人ブログのレビュー記事を執筆してください。
...
4. 商品リンクは, 文末付近に自然な形で `[楽天市場で「{clean_title}」の価格や在庫をチェックする]({url})` のようにMarkdownのリンク記法で埋め込んでください。
